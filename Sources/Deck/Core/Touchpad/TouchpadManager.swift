...
public class TouchpadManager: ObservableObject {
    ...
    fileprivate func processFrame(touches: UnsafeMutablePointer<MTTouch>, count: Int, timestamp: Double) {
        ...
        for i in 0..<count {
            let touch = touches[i]
            // state >= 2 表示有效接触在板上的手指
            if touch.state >= 2 && touch.state <= 4 {
        ...
        for (id, touch) in currentTouches {
            if activeFingers[id] == nil {
                activeFingers[id] = FingerInfo(
                    id: id,
                    startX: touch.normalized.x,
                    startY: touch.normalized.y,
                    currentX: touch.normalized.x,
                    currentY: touch.normalized.y,
                    startTime: now
                )
            } else {
                activeFingers[id]?.currentX = touch.normalized.x
                activeFingers[id]?.currentY = touch.normalized.y
            }
        }

        // 记录多指轻敲历史
        if currentTouches.count > multiTouchHistoryCount {
            multiTouchHistoryCount = currentTouches.count
            ...
            for lifted in liftedFingers {
                ...
                if duration < 0.28 && moveDistanceX < 0.05 && moveDistanceY < 0.05 {
                    ...
                    if currentTouches.count == 1 {
                        ...
                        if let restingInfo = activeFingers[restingTouch.identifier] {
                            // 固定手指必须已经按住至少 80ms
                            if now - restingInfo.startTime >= 0.08 {
                    ...
                    // 情况 2: 剩下 2 根手指固定按住 -> TipTap (3 Fingers Fix)
                        ...
                        let restingXs = restingTouches.map { $0.normalized.x }
